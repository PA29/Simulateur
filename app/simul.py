...
def run_simul(grid, json):   
    ...
    return({"heures":times, "buses":buses, "lines":lines, "P":P, "Q":Q, "U":V, "theta":theta, "I":[[[np.real(xi) for xi in x] for x in i] for i in I], "real(Sl)":[[[np.real(xi) for xi in x] for x in sl] for sl in Sl], "real(S)":[[[np.real(xi) for xi in x] for x in s] for s in S]})
